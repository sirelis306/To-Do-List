...
export class ChatList implements OnInit {
  ...
  public contextMenuConversationId: number | null = null;
  
  // Group Name Modal
  public isGroupNameModalOpen: boolean = false;
  ...
  cargarConversaciones(): void {
    this.chatService.getConversations().subscribe(
      (data) => {
        ...
        // Cargar el último mensaje para cada conversación
        this.conversations.forEach(conv => {
          // Si el backend no envía el lastMessage por defecto, lo consultamos:
          if (!conv.lastMessage) {
            this.chatService.getMessages(conv.id, 1).subscribe(
              (messages) => {
                if (messages && messages.length > 0) {
                  // Tomamos el único mensaje devuelto (el más reciente)
                  conv.lastMessage = messages[messages.length - 1];
                }
              },
              (err) => console.error(`Error cargando último mensaje para chat ${conv.id}`, err)
            );
          }
        });
      },
  ...
  onContextMenu(event: MouseEvent, conv: Conversation): void {
    event.preventDefault();
    this.contextMenuPosition = { x: event.clientX, y: event.clientY };
    this.contextMenuConversationId = conv.id;
  ...
  markAsRead(): void {
    if (this.contextMenuConversationId) {
      const id = this.contextMenuConversationId;
  ...
  openGroupNameModal(): void {
    ...
    this.isGroupNameModalOpen = true;
    this.newGroupName = '';
  }

  closeGroupNameModal(): void {
    this.isGroupNameModalOpen = false;
    this.newGroupName = '';
  }
  ...
  createGroupChat(): void {
    ...
    // Usamos el nombre del modal, o un default si está vacío
    const finalName = this.newGroupName.trim() ? this.newGroupName.trim() : 'Nuevo Grupo';

    this.chatService.createConversation('group', finalName, this.selectedUserIds).subscribe(
      (response) => {
        this.closeGroupNameModal();
        this.goBack();
        this.cargarConversaciones();
